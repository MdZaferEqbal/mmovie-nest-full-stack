...
function returnMovieDetails(url) {
    fetch(url).then(res => res.json()).then(function(data) {
        ...
        if ( data.length != 0 ) {
            ...
            movieTitleText.innerHTML = `${movieTitle} (${data.release_date.substring(0, 4)})
                <a class="tmdb-movie-link" href="https://www.themoviedb.org/movie/${data.id}" target="_blank">Tmdb <i class="fa-solid fa-link"></i></a>
            `;
            ...
            subHeading.innerHTML = `
                ...
                &bull;
                <span class="full-release-date">${data.release_date.split("-").reverse().join("/")}</span>
                &bull;
                <span class="genres">
            ...
            movieDetails.innerHTML = "<h1 class='no-movies-found'>No Details Found!</h1>"; 
        }
    });
}
...
function editReview(editBtn) {
    ...
    setTimeout(() => {
        document.getElementById(`go-back-btn-${id}`).addEventListener('click', function() {
